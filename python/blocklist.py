...
class BlocklistManager:
        ...
        if os.path.exists(CONFIG_PATH):
            try:
                with open(CONFIG_PATH, 'r') as f:
                    ...
                    self.regex_blocked = config.get('regex_blocked', [])
    ...
    def save_config(self):
        config = {
            ...
            'regex_blocked': self.regex_blocked,
    ...
    def load_blocklists(self):
        ...
        for filename in os.listdir(BLOCKLIST_DIR):
            try:
                with open(os.path.join(BLOCKLIST_DIR, filename), 'r', encoding='utf-8') as f:
                    for line in f:
                        domain = line.strip()
    ...
    def is_blocked(self, domain, client_ip=None):
        ...
        if domain in self.custom_blocked:
            return True
        for pattern in self.regex_blocked:
            try:
    ...
    def resolve_local(self, domain):
        return self.local_dns.get(domain)
        ...
        for url in self.blocklists:
            try:
                ...
                if r.status_code == 200:
                    ...
                    with open(os.path.join(BLOCKLIST_DIR, filename), 'w', encoding='utf-8') as f:
                        f.write(r.text)
            except Exception as e:
