...
def check_auth():
    if not manager.dashboard_password: return True
...
def login():
    if request.method == 'POST':
        if request.form.get('password') == manager.dashboard_password:
...
def import_config():
    ...
    if file:
        content = file.read()
        with open(CONFIG_PATH, 'wb') as f: f.write(content)
        manager.load_config()
...
def get_stats():
    ...
    if manager.privacy_level == "HIDE_ALL": return total, blocked, [], [], []
    ...
    conn.close()
    return total, blocked, top_blocked, top_allowed, top_clients

@app.route('/')
def dashboard():
    total, blocked, top_blocked, top_allowed, top_clients = get_stats()
    return render_template('dashboard.html', total=total, blocked=blocked, top_blocked=top_blocked, top_allowed=top_allowed, top_clients=top_clients)

@app.route('/statistics')
def statistics(): return render_template('statistics.html')

@app.route('/settings')
def settings(): return render_template('settings.html', config=manager.__dict__)

@app.route('/logs')
def logs():
    conn = get_db_conn()
    c = conn.cursor()
    c.execute("SELECT * FROM queries ORDER BY timestamp DESC LIMIT 100")
    log_data = [dict(row) for row in c.fetchall()]
    conn.close()
