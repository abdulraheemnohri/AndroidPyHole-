...
pub async fn run_server(blocklist: Arc<Mutex<Blocklist>>) {
    ...
    let urls = warp::path("urls").and(warp::get()).then(move || {
    ...
    let toggle_parental = warp::path("parental").and(warp::post()).then(move || {
    ...
    let export = warp::path("export").map(|| {
    ...
    let routes = stats.or(set_profile).or(urls).or(logs).or(toggle_parental).or(sync).or(export);
    println!("PyHoleX API Gateway active on 127.0.0.1:8080");
